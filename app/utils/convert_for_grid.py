...
xo = 210                 ##  기준점 X좌표
yo = 675                 ##  기준점 Y좌표
xo_grid = 210 / grid     ##  기준점 X좌표
yo_grid = 675 / grid     ##  기준점 Y좌표
first = 0
...
def mapToGrid(lat, lon, code = 0 ):
    ...
    if theta < -PI :
        theta += 2.0 * PI
    theta *= sn
    x = (ra * math.sin(theta)) + xo_grid
    y = (ro - ra * math.cos(theta)) + yo_grid
    x = int(x + 1.5)
    y = int(y + 1.5)
    return x, y
...
print(mapToGrid(37.64187065155636, 127.01685685267006))
